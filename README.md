...
클릭률 예측 문제에서는 단일 변수의 주효과보다는 변수들 간 교호작용 효과를 모델링하는 것이 예측력을 개선하는데 효과적이다. 하지만 교호작용 행렬을 직접 추정하기에는 데이터가 희소하다는 한계점이 있었다. 클릭률 예측 모형의 원형이 되는 팩토리제이션 머신(Factorization Machine)은 교호작용 행렬을 저차원 좌표계로 분해(Matrix Factorization)함으로써 문제를 해결하였다. 이후 클릭률 예측 모형은 교호작용 항의 모델링에 중점을 두고 발전해왔다. 후속 모형들의 주요 차이점은 첫째, 저차 교호작용을 각각 모델링하여 집계하는가 혹은 고차 교호작용을 직접 모델링하는가, 둘째, 귀납적 편향을 제공하는가로 요약될 수 있다. 여기서 귀납적 편향(Inductive Bias)이란 교호작용 메커니즘에 관한 사전 가정으로서, 주로 쌍선형(Bi-Linear) 사상이 가정되고 있다.

**본 연구는 이 귀납적 편향의 유무가 모델링 대상의 복잡도에 따라 예측력에 미치는 효과를 확인하고자 하였다.** 구체적으로, FM(Rendle, 2010), DeepFM(Guo et al., 2017), xDeepFM(Lian et al., 2018)에서 제안하는 교호작용 항에 대하여 다루었다. 이들 모형에서 모델링하는 교호작용 효과와 그 가정을 구분하여 각 모형을 다음과 같이 정의하였다. 즉, (1) 귀납적 편향 하에 2차 교호작용을 모델링하는 모형, (2) 귀납적 편향 없이 고차 교호작용을 모델링하는 모형, (3) 귀납적 편향 하에 고차 교호작용을 모델링하는 모형이다. 이 구분을 기반으로 모델링 대상에 따라 귀납적 편향이 예측력에 미치는 영향을 실험적으로 분석하였다. 실험 결과, 2차 교호작용을 모델링할 때는 단순화된 가정 하에서도 추정 안정성이 확보되었다. 하지만 고차 교호작용을 모델링할 때는 가정이 부재할 때 예측력이 개선되었다. 반대로 쌍선형 사상을 귀납적 편향으로 제공할 경우, 제공된 귀납적 편향이 교호작용의 복잡도를 충분히 반영하지 못하여 예측력이 저하되었다. **따라서 교호작용이 복잡할수록 사전 정의된 귀납적 편향을 제공하는 것보다 관측값에 기반하여 매칭 함수를 학습하는 것이 예측력 향상에 효과적이다.**

종합하자면, 쌍선형 사상을 가정하는 모형은 연산 과정이 명시적(Explicit)이라는 점에서 분석 목적에는 적합하지만, 가정의 제약으로 인하여 표현력에 한계가 있어 예측력을 기대하기 어렵다는 것이 일반적인 통념이다. 그러나 귀납적 편향에 상응하는 변수 설계가 수반되는 경우, 외부 교란에도 강건한 예측력을 확보할 수 있음을 확인하였다. 또한, 귀납적 편향을 부여하지 않는 인공신경망 기반 모형은 높은 자유도로 인하여 과적합 가능성이 제기되어 왔다. 그러나 모델링 대상의 복잡도가 높은 경우에는 이 자유도에 기반하여 데이터로부터 교호작용 메커니즘을 학습함으로써 희소한 환경에서도 우수한 예측력을 보일 수 있음을 확인하였다. **이상의 결과는 귀납적 편향에 적합한 변수 설계가 이루어진 경우 강건한 예측력을 기대할 수 있지만, 그렇지 못한 경우에는 데이터로부터 그 구조를 학습하는 컴포넌트를 도입하는 것이 효과적임을 시사한다.**
...
본 연구에서 다루는 귀납적 편향은 구체적으로 두 변수(혹은 다변수)의 교호작용이 쌍선형적으로 이루어진다(Bilinear Interaction)는 가정이다. 따라서 귀납적 편향이 예측력에 미치는 효과를 실험하기 이전에, 이 쌍선형 교호작용을 이해하고자 한다. 우선, 선형 공간(Linear Space)이 정보를 여러 의미 단위들로써 분해하여 기술하는 하나의 의미 체계라 정의하자. 선형성(Linearity)은 가법성(Additivity)과 동차성(Homogeneity)의 공리를 만족하는 성질이다.

여기서 가법성이란 의미 단위들 간에 교섭이 발생하지 아니하여 특정 단위로 사상된(평가된) 정보가 다른 단위들과 독립적으로 처리됨을 의미한다. 예컨대 어떤 물체의 물리량을 측정할 때 센티미터(centimeter) 단위와 킬로그램(kilogram) 단위는 상호간에 의존하지 않고 독립적으로 측정되는 것과 같다. 또한 동차성은 단위의 방향이 휘어지거나(Sigmoid) 차단되지(ReLU) 아니하여 특정 단위로 사상된(평가된) 정보가 스케일의 변화에 의해 왜곡되지 아니하고 보존됨을 뜻한다. 이는 $100cm$ 길이의 물체를 측정할 때와 $100,000cm$ 를 측정할 때 센티미터(centimeter) 단위 길이가 바뀌지 않는 것과 같다.

종합하자면, 선형성이란 선형 공간의 기저, 즉 정보를 기술하는 의미 체계의 기본 단위들에 대한 가정으로서, 이들 단위가 변형되지 아니하고 상호간에 의존하지 않아 독립적으로 유지되는 성질로 해석될 수 있다. 쌍선형 관계(Bi-Linear Relationship)란 두 변수 $\mathbf{u},\mathbf{v}$ 중 어느 하나를 고정하였을 때 다른 하나가 선형이 되는 관계를 말한다. 이 관계는 공간 $\mathbf{u}\in \mathcal{U}$ 와 $\mathbf{v}\in \mathcal{V}$ 가 모두 선형 공간이고, 서로 선형 변환으로 사상될 수 있다고 가정한다. 즉, $\mathcal{U}$ 의 기저는 $\mathcal{V}$ 의 기저들의 선형 결합으로 표현될 수 있으며, 반대로 $\mathcal{V}$ 의 기저 역시 $\mathcal{U}$ 의 기저들의 선형결합으로 표현될 수 있다. 이는 두 공간이 동일한 선형 구조를 공유함을 의미한다.
...
직관적으로 이해하자면, 쌍선형 관계는 사용자를 기술하는 의미 단위들이 광고를 기술하는 의미 단위들을 일정 비율로 조합한 결과로써 설명될 수 있음을 뜻한다. 다시 말해, 두 엔티티를 기술하는 의미 단위들은 서로 무관하지 않고 하나의 공통된 의미 체계를 서로 다른 관점에서 기술한 것에 불과하다. 따라서 이 관계 구조 하에서 두 엔티티 간 교호작용 효과는 교호작용만을 설명하기 위한 새로운 의미 체계를 요청하지 않고서도, 이미 공유하고 있는 의미 체계 위에서 직접 평가될 수 있다. 쌍선형 교호작용 함수의 일반형에서 $\mathbf{W}$ 는 벡터 $\mathbf{v}$ 를 $\mathbf{u}$ 의 공간으로 사상하는 선형 변환 행렬이다. 내적에서 $\mathbf{W}$ 는 항등 행렬이 된다. 이는 두 변수가 사상되어 있는 선형 공간이 동일함을 전제한다.
...
팩토리제이션 머신 및 그 후속 모형들의 기본 형태는 위 수식과 같다. 여기서 $\beta_{0}$ 은 편향, $\sum_{i}{\beta_{i}x_{i}}$ 은 변수의 주효과, $f(X)$ 는 변수들 간 교호작용 효과를 나타낸다. 본 연구에서는 Factorization Machine(Rendle, 2010), Deep Factorization Machine(Guo et al., 2017), eXtreme Deep Factorization Machine(Lian et al., 2018) 를 살펴보고자 한다.
...
위 수식은 Factorization Machine(Rendle, 2010)에서 제안하고 있는 교호작용 효과인 `F`actorization `M`achine(FM)이다. 이는 두 개 변수의 교호작용을 내적(Inner Product)으로 모델링한다. 따라서 해당 모형은 교호작용 메커니즘을 쌍선형 사상으로 사전 규정하여 2차 효과만을 모델링하고 있음을 확인할 수 있다.
...
위 수식은 Deep Factorization Machine(Guo et al., 2017)에서 제안하고 있는 교호작용 효과인 `D`eep `N`eural `N`etworks(DNN)이다. 이는 다변수(multivariable)의 정보를 기저별로 누적(`sum`)하거나 선택(`mean`)하지 않고 결합한 후, 그 교호작용을 `M`ulti-`L`ayer `P`erceptron(MLP)로 모델링한다. 따라서 해당 모형은 교호작용 메커니즘에 대한 사전 가정 없이 고차 효과를 모델링하고 있음을 확인 가능하다.
...
위 수식은 eXtreme Deep Factorization Machine(Lian et al., 2018)에서 제안하고 있는 교호작용 효과인 `C`ompressed `I`nteraction `N`etworks(CIN)이다. 초기 단계에서는 성립 가능한 모든 변수 쌍에 대하여 내적(Inner Product)을 수행한다. 이후 모든 쌍에 대한 `channels` 개의 선형 결합을 도출한다($1\times 1$ Convoluation). 합성곱 계층의 수가 총 $N$ 장이라고 한다면, CIN은 총 $N+1$차 교호작용의 선형 결합을 명시적으로 도출하게 된다. 요컨대 해당 모형은 교호작용 메커니즘을 쌍선형 사상으로 사전 규정하여 고차 효과를 모델링한다고 볼 수 있다.
...
첫째, 클릭률 예측 문제에서는 미클릭 샘플의 출력값 분포를 안정적으로 형성하는 것이 예측 성능 향상에 직결된다. 단, 미클릭 샘플의 클릭률을 낮게 추정하는 것이 반드시 성능 향상으로 귀결된다고 볼 수 없다. 그 근거로, DNN 계열 모형에서는 미클릭 샘플의 출력값 분포가 점차 완만해질수록 추정 안정성이 확보되고 성능이 개선된 반면, CIN 계열 모형에서는 미클릭 샘플을 낮은 클릭률 영역에 밀집시킬수록 과소추정의 문제가 발생하여 성능이 저하되었다. 특히 과소추정이 가장 두드러졌던 64차원에서 CIN의 성능이 크게 하락하였다. 과대추정 경향을 보인 eXtremeDNN 계열 모형이 CIN 계열 모형보다 성능이 우수하였음을 고려했을 때, 과소추정이 과대추정보다 성능 저하로 귀결될 여지가 크다.
...
둘째, 교호작용 항을 임베딩 표현 학습(Representation Learning)과 매칭 함수 학습(Matching Function Learning)의 두 컴포넌트로 해석할 수 있다. 이때 모형의 예측력은 교호작용의 복잡도가 증가할수록 임베딩 표현 학습보다 매칭 함수 학습에 좌우되는 경향이 있다. 표현 학습(Representation Learning)은 고차원 관측 공간으로부터 데이터가 형성하는 잠재 구조(Latent Structure)를 포착하고, 이를 효과적으로 표현할 수 있는 저차원 좌표계(Latent Space)를 학습하는 것을 목적으로 한다. 그런데 클릭률 예측 문제에서 관측 공간은 개별 엔티티가 아니라 사용자, 아이템 및 컨텍스트 등 다양한 엔티티 간의 교호작용 결과값이 된다. 이때 표현 학습은 관측된 교호작용으로부터 각 엔티티의 잠재요인(Latent Factor)을 추론하는 과정이 된다. 여기서 잠재요인은 다른 엔티티와의 관계를 생성하는 요인으로 이해될 수 있다. 

FM과 CIN은 모두 요소별 곱셈(Element-wise Product)만으로 엔티티들의 교호작용을 모델링한다. 이때 요소별 곱셈(Element-wise Product)은 교호작용에 대한 쌍선형(Bi-Linear) 가정을 전제한다. 다만, FM은 2차 교호작용만을 모델링하는 반면, CIN은 동일한 연산 구조를 반복하여 고차 교호작용을 모델링한다. 이처럼 교호작용 모델링 함수가 특정되었을 때는 예측력이 학습된 임베딩 표현의 품질에 전적으로 의존하게 된다. 이는 함수가 학습 대상이 아니라 연구자가 사전에 규정한 귀납적 편향으로 작용하기 때문이다. 반면, DNN은 고차 교호작용 메커니즘을 사전에 가정하지 않고, 데이터로부터 이를 모델링하는 함수를 직접 학습한다.
...
[^1]: 본 연구는 초기 교호작용 항인 FM, DNN, CIN을 조합하고 그 성능을 비교하는 데 그쳤다. 수정 기간 동안 연구 목적 재설정, 모형 및 실험 파이프라인 재설계 후 실험을 재수행하였다.